...
# Reuse existing nodebuilder-wallos container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-wallos; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-wallos -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/bellamy/wallos:${APP_VERSION}" \
    "${container}"
# Commit the image
